// [2026-01-29] 추가: 로그인 라우트 및 인증 가드 - 로그인하지 않으면 접속 불가
...
import Login from '@/custom/pages/Login.vue';
...
export async function createAppRouter(): Promise<Router> {
    ...
    router.beforeEach(async (to, from, next) => {
        // [2026-01-29] 추가: 인증 체크 - 로그인하지 않으면 로그인 페이지로 리디렉션
        ...
        // 이미 인증된 사용자가 로그인 페이지에 접근하면 홈으로 리디렉션
        if (isAuthenticated && isLoginPage) {
            next({ path: '/' });
            return;
        }
